...
<body>
    ...
    <header><h2>Edit Post</h2></header>
<div class="container">
    <h2>Edit Article {{$post['title']}}</h2>
    ...
    <form action="{{route('updatePost',$post['id'])}}" method="post" enctype="multipart/form-data">
        ...
        <input type="text" id="genre" name="tag" value="{{$post['tag']}}">
        @error('tag')
            <p class="text-danger">{{$message}}</p>
        @enderror
    ...
    @enderror
        ...
        <textarea id="content" name="content" rows="8" >{{$post['content']}}</textarea>
        @error('content')
        <p class="text-danger">{{$message}}</p>
    @enderror
